...
- Domain Events + Outbox Pattern: Persist side-effect events with transactional writes and publish them asynchronously. We use it for post-payment notifications, unlock scheduling, and audit-related events because it solves lost-event and cross-system consistency issues. Implemented for realtime notification events: producers write an `OutboxEvent` row in the same transaction as the state change (`apps/api/src/common/outbox`), and an in-process `OutboxDispatcherService` polls pending rows and publishes them at-least-once through registered handlers. The dispatcher runs inside `apps/api` because it must reach the in-process Socket.IO server; moving the publisher to `apps/workers` with multi-instance-safe claiming (`FOR UPDATE SKIP LOCKED`) is gated on the Redis Socket.IO adapter (#76). Payment/wallet side effects are a planned extension.
